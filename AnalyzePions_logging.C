...
TLorentzVector angular(0.0,1.0, 0.0, 0.0);
...
float lab (TVector3 *mom_proton, TVector3 *angular_boost, TVector3 *b_b);	// Pass it the 3 vector, construct the spin 4 vector, boost it to lab frame and dot it
TVector3 lboost (TVector3 *b_b); // function boosts and returns Lprime
void AnalyzePions_logging ();
{
//	ofstream log;
//	log.open("phi.dat");
	TStopwatch timer;
	...
	TH1F *dndphi = new TH1F ("hits per phi", "hits per phi", 20, 0 , 2.*pi);
	TProfile *pols_phi = new TProfile ("polarization versus phi", "polarization versus phi", 2000, 0, 2.*pi);
	float beta = 0.95;
...
// to edit for L prime hat somehow add in an average dot product.
	for(int i = 0; i <= i_max; i++)
	{
		...
		for(float phi = 0.0; phi < 2.*pi; phi += pi/1000.)
		{
			...
			pols_1->Fill(2.5,lab(&mom_proton, &angular_boost, &b_b));
			...
			pols->Fill(2.5,lab(&mom_proton, &angular_boost, &b_b));
	...
	pols_phi->Draw();
	c1->SaveAs("isaac_pols_vs_phi_cut.pdf");
...
float lab (TVector3 *mom_proton, TVector3 *angular_boost, TVector3 *b_b) // both are in lab frame, boost spin to lab frame, construct the spin 4-vector, boost back to lab frame, dot product
{
	float polarization = 0.0;
...
//	TVector3 angular(0.0,1.0, 0.0);
	polarization = spin.Dot(angular_boost)/(spin.Mag()*angular_boost.Mag());
	return polarization;
}
